...
st.markdown("""
The molecular system examined in this study was constructed using the recently published cryo-EM structure[^2], AgOR28 PDB: 8V3C, and its AlphaFold2 model for missing residues (residues 1-7, 155-174, and 240-321). The protonation states of ionizable residues were set according to their predicted pKa values at physiological pH (6.5), as determined by PROPKA3[^6]. The system was embedded into a POPC/12% cholesterol model using PACKMOL-memgen[^7]. Water molecules and relevant ions were incorporated to mimic physiological conditions, with a water slab with a thickness of at least 17.5 Å above and below the membrane. Explicit solvation was carried out with the TIP3P water model[^8] for all systems. The total system is made up of 380278 atoms in a periodic box of 168*168*158 Å[^3]. Systems were neutralized with the addition of Na[^+] or Cl[^−] ions, with a ﬁnal concentration of 0.15 M NaCl to emulate physiological ionic strength. All molecular dynamics (MD) simulations were performed using the AMBER ff19SB[^9] force field (FF) for proteins. Ligands geometries were optimized with Gaussian1610 (B3LYP[^11-13]/6-31G*[^14]) and electrostatics were calculated at the HF/6-31G* level of theory. Ligands parameters and partial charges were derived using the RESP method[^15], as implemented in the Antechamber module. The other parameters were taken from the General Amber Force Field 2 (GAFF2). Lipid bilayers were described with the Lipid21 force field. All topologies and parameter files were assembled using the tleap module in AmberTools[^16]. 
 
The prepared system underwent a nine-stage energy minimization protocol using Amber24[^17]. First, solvent molecules and ions were relaxed while solute atoms, ligand, protein, and bilayer membrane were restrained with a 20 kcal/(mol*Å) force constant for 10000 steps, 5000 of which are steepest descent while the rest of the steps were conjugate gradient minimization. This was followed by a 10 ps NVT ensemble equilibration at 100 K of the entire system with the same restraints. Cycling between this minimization and equilibration in this manner was done four times, lessening the restrains by 5 kcal/(mol*Å) each cycle and increasing the timestep from 1 fs to 2 fs (and the MD time to 20 ps) for the remaining three cycles, and additionally only restraining the ligand and protein for the fourth cycle. This was followed by the minimization of the entire system without restraints. 
Equilibration was performed in six steps with GROMACS[^18]. Three 150 ps NVT ensemble at 300 K using restrains on the backbone, sidechain, and phosphor atoms of the lipid (only in the z direction, perpendicular to the surface of the membrane), with 1) 96, 49, 24; 2) 49, 24, 9.6; 3) 24, 12, 9.6 kcal/(mol*Å) force constants, respectively ; 3-6) Three 500 ps NPT ensemble at 1 bar and 300 K using the same restraints as before with 1) 12, 4.7, 4.7; 2) 4.7, 1.2, 1; 3) 1.2, 0, 0 kcal/(mol*Å) force constants, respectively.
All equilibration steps employed a 2 fs time step and The LINCS algorithm was used to constrain all bonds involving hydrogen[^19]. Production MD simulations were conducted in the NPT ensemble at 1 bar and 300 K under periodic boundary conditions with a leapfrog integration scheme and a time step of 2 fs. Protein, Lipid and solvated molecules (including the ligand) were coupled to three separate thermostats at 300 K using a weak coupling scheme and a relaxation time of 0.1 ps. The temperature was held fixed using the V-rescale thermostat[^20] with a coupling time of 0.1 ps. The pressure was kept in all simulations at the reference pressure of 1 bar, exploiting the features of the C-rescale barostat[^21] with a coupling time of 2 ps and an isothermal compressibility of 4.5 × 10–5 bar–1 . Long-range electrostatics were treated with the Particle Mesh Ewald (PME) method, with a real-space cutoff of 10 Å. A nonbond pair list with cutoff of 1.4 nm was created and updated every 200 steps and van der Waals interactions were truncated at 10 Å with a switching function applied from 8 Å. Simulation trajectories were processed and analyzed using GROMACS built-in tools, and MDAnalysis python library[^22]. Root-mean-square deviation (RMSD), root-mean-square fluctuation (RMSF), and radius of gyration (Rg) were calculated for backbone atoms to monitor protein stability.
 
Quantum Mechanics/Molecular Mechanics (QM/MM) single point energy (SPE) calculations were performed using the quantum chemistry program ORCA[^23] and r2SCAN[^24]/TZVPP[^25,26] basis set was used with a D4 dispersion correction[^27,28]. The 2,4,5-Trimethylthiazole (TMT) agonist and surrounding environment—residues 75,76,79,140,141,196,199,204 as numbered in the cryo-EM structure—were treated with QM (QM region) while the rest of the protein was treated as point charges.
""")
